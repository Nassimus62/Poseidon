      ...
      complete: (results) => {
        try {
          ...
          if (firstRow.length < 2) {
            throw new Error('File must contain at least 2 columns (timestamp and sea level)');
          }
          ...
          results.data.forEach((row: any, index: number) => {
            ...
            if (!timestamp || isNaN(timestamp.getTime())) {
              errors.push(`Row ${index + 1}: Invalid timestamp format "${timestampStr}"`);
              return;
            }
            ...
            if (isNaN(seaLevel)) {
              errors.push(`Row ${index + 1}: Invalid sea level value "${seaLevelStr}"`);
              return;
            }
          ...
          if (data.length < 10) {
            errors.push('Warning: File contains very few data points, analysis may be limited');
          }
          ...
          const fileData: FileData = {
            filename: file.name,
            data,
            isValid: errors.length === 0 || errors.every(e => e.startsWith('Warning')),
            errors
          };
      ...
      <CardContent>
          ...
          {isProcessing ? (
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Accepts .CSV or .TXT files with timestamp and sea level columns
              </p>
        ...
        {uploadStatus === 'success' && (
          <Alert className="mt-4">
            ...
            <AlertDescription>
              File uploaded successfully! {acceptedFiles[0]?.name} loaded with data ready for analysis.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="mt-6 p-4 bg-muted/50 rounded-lg">
          ...
          <ul className="text-sm text-muted-foreground space-y-1">
            ...
            <li>• Timestamps: YYYY-MM-DD HH:MM or HH:MM format</li>
            <li>• Sea levels: numeric values (meters or centimeters)</li>

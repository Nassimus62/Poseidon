...
export class PoseidonAnalysisEngine {
  ...
  private async detectTidalPhases(data: DataPoint[], tidal: DataPoint[]): Promise<DetectedEvent[]> {
    const events: DetectedEvent[] = [];
    const threshold = 2; // cm threshold for tide change detection
    ...
    for (let i = 1; i < tidal.length - 1; i++) {
      ...
      if (prevSlope > threshold && nextSlope < -threshold) {
        events.push({
          id: `high-tide-${i}`,
          type: 'High Tide',
          label: `High Tide (${curr.seaLevel.toFixed(1)} cm)`,
          startTime: curr.timestamp,
          confidence: 'High',
          amplitude: curr.seaLevel,
          explanation: `Peak detected with incoming slope ${prevSlope.toFixed(1)} cm/min and outgoing slope ${nextSlope.toFixed(1)} cm/min`,
      ...
      if (prevSlope < -threshold && nextSlope > threshold) {
        events.push({
          id: `low-tide-${i}`,
          type: 'Low Tide',
          label: `Low Tide (${curr.seaLevel.toFixed(1)} cm)`,
          startTime: curr.timestamp,
          confidence: 'High',
          amplitude: curr.seaLevel,
          explanation: `Trough detected with incoming slope ${prevSlope.toFixed(1)} cm/min and outgoing slope ${nextSlope.toFixed(1)} cm/min`,
          properties: { level: curr.seaLevel, slope_in: prevSlope, slope_out: nextSlope }
        });
      }
      ...
      if (Math.abs(prevSlope) > threshold) {
        ...
        events.push({
          ...
          type,
          label: `${type} (${Math.abs(prevSlope).toFixed(1)} cm/min)`,
          startTime: prev.timestamp,
          endTime: curr.timestamp,
          confidence,
          explanation: `Tidal ${type.toLowerCase()} detected with slope ${prevSlope.toFixed(2)} cm/min`,
        ...
        if (surgeStart !== null) {
          ...
          if (duration >= sustainedDuration) {
            ...
            events.push({
              ...
              type: 'Storm Surge',
              label: `Storm Surge (${maxSurge.toFixed(1)} cm peak)`,
              startTime: residuals[surgeStart].timestamp,
              endTime: residuals[i - 1].timestamp,
              peak: maxSurgeTime!,
              confidence,
              amplitude: maxSurge,
              explanation: `Sustained surge detected with ${maxSurge.toFixed(1)} cm peak amplitude over ${duration} minutes`,
              properties: { 
  ...
  private async detectDataArtifacts(data: DataPoint[]): Promise<DetectedEvent[]> {
    ...
    // Detect spikes
    const spikeThreshold = 50; // cm sudden change
    for (let i = 1; i < data.length; i++) {
      ...
      if (change > spikeThreshold) {
        events.push({
          id: `spike-${i}`,
          type: 'Spike',
          label: `Data Spike (${change.toFixed(1)} cm jump)`,
          startTime: data[i].timestamp,
          confidence: 'High',
          amplitude: change,
          explanation: `Sudden level change of ${change.toFixed(1)} cm detected between consecutive measurements`,
    ...
    // Detect flatlines
    const flatlineThreshold = 0.1; // cm
        ...
        if (flatlineStart !== null) {
          ...
          if (duration >= minFlatlineDuration) {
            events.push({
              ...
              type: 'Flatline',
              label: `Data Flatline (${duration} min at ${flatlineValue!.toFixed(1)} cm)`,
              startTime: data[flatlineStart].timestamp,
              endTime: data[i-1].timestamp,
              confidence: 'High',
              explanation: `Constant value ${flatlineValue!.toFixed(2)} cm maintained for ${duration} minutes`,
              properties: { 
    ...
    data.forEach(point => {
      ...
      if (isExtreme) {
        ...
        events.push({
          ...
          type: 'Extreme Level',
          label: `Extreme ${type} Level (${point.seaLevel.toFixed(1)} cm)`,
          startTime: point.timestamp,
          confidence,
          amplitude: Math.abs(point.seaLevel - mean),
          explanation: `Level ${point.seaLevel.toFixed(1)} cm exceeds ${confidence.toLowerCase()} threshold (${threshold} cm from mean)`,
          properties: { 
      ...
      if (nyquistPower > meanPower * 3) {
        ...
        highVariancePeriods.forEach((period, index) => {
          events.push({
            ...
            label: `Probable Aliased Waves (${period.variance.toFixed(2)} variance)`,
            startTime: period.start,
            endTime: period.end,
            confidence: 'Low',
            explanation: `High variance (${period.variance.toFixed(2)}) and energy near Nyquist frequency (${nyquistFreq} cyc/min) suggest aliased short-period waves`,
            properties: { 
